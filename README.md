...
下面依次是 **现价**（能识别出标的时）、**同题报道与网上相似观点**，以及结构化 **AI 分析**（板块 / 概念 / 相似观点 / 看多概率 / 看空概率 / 传导逻辑）。
...
4. **证券 AI 分析** —— DeepSeek 收到的不是孤立标题，而是「新闻事实 + 已核验现价/涨跌幅 + 本地词典匹配的行业板块/概念 + 同题报道 + 网上相似观点」。每条严格输出六项：**【板块】【概念】【相似观点】【看多】【看空】【逻辑】**。看多/看空是未来 **1–5 个交易日的事件影响情景概率**，两者必须合计 100%，必须同时解释支撑与风险；证据不足时应接近 50%/50%。逻辑必须写成「事件 → 业务/供需/现金流/估值或风险偏好 → 板块/个股」传导链。概率是有限证据下的主观权重，不是统计预测、目标价或收益承诺。

输出仍受事实与合规约束：只能引用输入里的数字、词典匹配、搜索标题与公开摘要；不能把「同标的消息」冒充同一事件，不能假装读过搜索链接全文。多空概率不合计 100%、荐股措辞越线、或单一来源却声称多方证实时，模型分析会被拒收。未配置 API Key 或调用失败时，系统按事件类型给出同样六字段的规则化分析，并注明「规则情景权重，非统计预测」；卡片只标「分析」，不假装用了 AI。

模型对每条情报的输出格式（`octopus/ai.py` 的 `NEWS_ANALYSIS_PROMPT`，`octopus/enrich.py` 按编号解析成两段）：

```
1. 一句话：大额回购短线偏多，但真正分量取决于执行规模和节奏。
   分析：【板块】电力设备；【概念】锂电池、新能源车；【相似观点】证券时报与财联社均确认回购事实，某券商解读强调信心修复，但观点来源仍有限；【看多】62%：回购改善筹码与信心预期；【看空】38%：价格上限、期限和实际执行比例待核验；【逻辑】回购计划 → 流通筹码与风险偏好变化 → 个股及相关概念定价，验证点是后续回购进展公告。
```

模型没按新格式写「一句话」时，整段仍按分析处理，开头那一句由规则化文本兜底——老格式输出不会因升级失效；新六字段格式一旦出现概率，则会校验看多与看空是否合计 100%。
...
AI 分析
      【板块】电力设备
      【概念】锂电池、新能源车
      【相似观点】两家同题报道确认事实；两条观点都重视信心信号，但对执行效果仍有分歧
      【看多】62%：回购改善筹码与信心预期
      【看空】38%：价格上限、期限和实际执行比例待核验
      【逻辑】回购计划 → 流通筹码与风险偏好变化 → 个股及相关概念定价；验证点为进展公告
```

页面上的实际排版：`标题` → **`一句话`（深底高亮，全页最突出）** → `现价` → `源摘要` → `多源/观点` → `AI 分析六字段` → `时间与标签`。看多用 A 股红、看空用绿，六项分行展示，便于手机扫读。
...
├── enrich.py         # 情报条目：现价、板块/概念匹配、六字段 AI/规则化证券分析
...
tests/                # 472 个测试，全离线（含多源观点、概率校验、一句人话与排版回归测试）
└── fixtures/         # 真实接口响应样本
```
...
472 个测试全部离线运行（用真实抓取的响应样本做 fixture），不依赖网络。

**情报抓取与证券分析**：时间解析的各种畸形格式（含 RSS 的 RFC 822 时间）、过滤逻辑、去重、降级路径、HTML 转义、
条数上限语义（0 = 全量）、每条新闻的一句人话与六字段分析（代码提取 / 东财→Yahoo 降级 /
行业与概念词典匹配 /「一句话 + 分析」两段解析 / 规则化概率兜底 / 多空概率合计 100 校验 /
荐股措辞拒绝 / 单一来源却声称多方证实的拒收 / 六字段移动端排版），以及手动主题分析与端到端流程。
