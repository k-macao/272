...
3. **一句人话（每条开头，页面突出显示）** —— 深底 + 荧光绿左边条，一句话讲清「这条消息意味着什么」，扫一眼就知道轻重。要求以**证券投资专家的口吻说大白话**（25–40 字），可以写短线偏多/偏空/中性，但不能写成操作指令；荐股、目标价、收益承诺同样禁止，**单一来源却写「已获多方证实」的句子会被拒收**。未配置 `DEEPSEEK_API_KEY` 或调用失败时，按事件类型给出规则化一句话，卡片上标「一句话」而不是「AI 一句话」。一句话与详细分析**各自过一遍合规红线**，一段越线不会连累另一段。
...
模型没按新格式写「一句话」时，整段仍按分析处理，开头那一句由规则化文本兜底——老格式输出不会因升级失效；五模块格式一旦出现概率，则会校验【多维推演】里的偏多与偏空是否合计 100%。
...
tests/                # 476 个测试，全离线（含多源观点、概率校验、一句人话与排版回归测试）
└── fixtures/         # 真实接口响应样本
```
...
476 个测试全部离线运行（用真实抓取的响应样本做 fixture），不依赖网络。
...
荐股措辞拒绝 / 单一来源却声称多方证实的拒收 / 五模块移动端排版），以及手动主题分析与端到端流程。
