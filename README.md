...
推送样式：浅灰底色 + 深色正文。每条情报附 **现价**（能识别出标的时）、**同一新闻的其它源头**（多源印证）和 **AI 分析**（事件要点 / 多源印证 / 关注点）。
...
抓取、时间校验、去重完成之后、渲染推送之前，对**每一条**留下的情报做三件事。失败只让该条缺字段，不会丢掉已经校验过的内容。
...
2. **多源印证** —— 找**同一新闻的不同源头**，让 AI 评论前先有比对对象（`octopus/crossref.py`）：
   ...
   - 找不到就如实标 **「单一来源」**，绝不凑数。
3. **AI 分析** —— 配置了 `DEEPSEEK_API_KEY` 时，把「本条 + 其它源头报道」一起交给 DeepSeek，按 ① 事件要点 ② 多源印证（各源是否一致、有无补充或口径差异；单一来源必须写明「待其它渠道确认」）③ 关注点 三段写 60–120 字中性分析。只能用给出的事实，禁止编造数据、禁止荐股；**单一来源却声称「已获多方证实」的输出会被拒收**。未配置或调用失败时降级为规则化分析（印证情况 + 按事件类型给出的核对要点），卡片上标「分析」而不是「AI 分析」，不假装用了大模型。
...
├── state.py          # 跨轮次去重（原子写入）
├── http.py           # 重试、超时、UA 伪装
├── ai.py             # DeepSeek 大模型：内容提炼 + 主题因子报告解读 + 新闻逐条 AI 分析
├── enrich.py         # 情报条目：提取代码、现价、多源印证编排、AI/规则化分析
...
tests/                # 437 个测试，全离线（含多源印证、合并排版与单条上限回归测试）
└── fixtures/         # 真实接口响应样本
```
...
437 个测试全部离线运行（用真实抓取的响应样本做 fixture），不依赖网络。

**情报抓取**：时间解析的各种畸形格式（含 RSS 的 RFC 822 时间）、过滤逻辑、去重、降级路径、HTML 转义、
条数上限语义（0 = 全量）、每条新闻的现价与 AI 分析（代码提取 / 东财→Yahoo 降级 /
规则化分析 / 荐股措辞拒绝 / 单一来源却声称多方证实的拒收）、手动主题分析推送（一对一语义 / 独立网页 / 令牌保护 / DeepSeek 提炼）与端到端流程。
