...
推送样式：浅灰底色 + 深色正文。每条情报附 **现价**（能识别出标的时）和 **一句总结**。
...
抓取、时间校验、去重完成之后、渲染推送之前，对**每一条**留下的情报做两件事。失败只让该条缺字段，不会丢掉已经校验过的内容。

1. **现价** —— 从 `extra.code` / 标题里的 `（600519）`、`300750.SZ`、公告证券代码、以及内置词典里的证券简称提取标的，批量取东财实时行情；东财失败或漏报再降级 Yahoo Finance。转债源内已有的 `price` 作为最后兜底。识别不出标的或行情缺失时**整行不展示**，绝不编造价格。上证指数 `000001` 与平安银行同码，只在「上证/沪指」语境下走指数。
2. **一句总结** —— 配置了 `DEEPSEEK_API_KEY` 时由 DeepSeek 按条生成（只根据标题与摘要，禁止编造数据、禁止荐股）；未配置或调用失败时降级为规则化摘要（源摘要首句，否则压缩标题），卡片上标「摘要」而不是「AI」，不假装用了大模型。

```
标题  宁德时代(300750) 拟回购
现价  宁德时代 300750  188.50  +2.31%
AI    公司拟斥资回购，关注后续进度。
```
...
- **兼容 AI**：如配 `DEEPSEEK_API_KEY`，合并后自动调用 DeepSeek 提炼摘要
- **移动端排版**：Markdown 标题、列表、引用、表格、代码块会转换为窄屏章节卡片
- **正文精简**：推送时隐藏目录、文件名、指纹及合并过程信息，只保留报告正文
- **单条推送**：正文上限 100,000 字符，始终只发送一条，不添加分页标题
...
├── state.py          # 跨轮次去重（原子写入）
├── http.py           # 重试、超时、UA 伪装
├── ai.py             # DeepSeek 大模型：内容提炼 + 主题因子报告解读 + 新闻一句总结
├── enrich.py         # 情报条目：提取代码、现价、AI/规则化一句总结
├── render.py         # 浅灰底 + 深色字的 HTML 渲染
...
tests/                # 358 个测试，全离线（含合并排版与单条上限回归测试）
└── fixtures/         # 真实接口响应样本
```
...
386 个测试全部离线运行（用真实抓取的响应样本做 fixture），不依赖网络。

**情报抓取**：时间解析的各种畸形格式、过滤逻辑、去重、降级路径、HTML 转义、
条数上限语义（0 = 全量）、每条新闻的现价与一句总结（代码提取 / 东财→Yahoo 降级 /
规则化摘要 / 荐股措辞拒绝）、手动主题分析推送（一对一语义 / 独立网页 / 令牌保护 / DeepSeek 提炼）与端到端流程。
