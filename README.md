...
推送样式：浅灰底色 + 深色正文。每条情报开头有一句**突出显示**的「AI 一句话」——投资专家口吻的大白话结论；
下面依次是 **现价**（能识别出标的时）、**同一新闻的其它源头**（多源印证）和 **AI 分析**（事件要点 / 多源印证 / 关注点）。
...
2. **多源印证** —— 找**同一新闻的不同源头**，让 AI 评论前先有比对对象（`octopus/crossref.py`）：
   - **本轮跨源匹配（离线，始终执行）**：十个源同一轮抓回来的条目互相比对。「共同标的 + 共同事件词」（回购/增持/问询函/涨停/CPI…共 25 类）判定为**同一事件**；只共享标的、事件不同且间隔 3 小时内的判定为**同标的**（卡片标「待核对」，不冒充同一事件）。比对标题重合度时先剥掉标的名与代码——「宁德时代(300750) 回购公告」与「宁德时代(300750) 触及涨停」光凭名字就能重合一大截，不能因此算同一件事。
   - **外部新闻检索（在线，可关）**：用 Google News / Bing News 的 RSS 检索「标的名 + 事件词」，找证券时报、财联社、新华社等媒体的同题报道。结果同样过时间校验：**没有发布时间、时间在未来、与原条目相差超过 36 小时的一律丢弃**；标题对不上的（标的不同 / 事件不同）丢弃；同一家源头（东财快讯搜出东财网）不算「不同源头」。每轮只对最值得核实的 12 条检索（有标的、有事件词、本轮尚无印证的优先），单次 6 秒超时不重试，连续两次失败自动熔断，不拖慢主流程。
   - 找不到就如实标 **「单一来源」**，绝不凑数。
3. **一句人话（每条开头，页面突出显示）** —— 深底 + 荧光绿左边条，一句话讲清「这条消息意味着什么」，扫一眼就知道轻重。要求以**投资专家的口吻说大白话**（25–40 字），明令禁用「利好情绪面」「估值修复空间打开」「量价共振」这类空话套话；荐股、目标价、收益承诺同样禁止，**单一来源却写「已获多方证实」的句子会被拒收**。未配置 `DEEPSEEK_API_KEY` 或调用失败时，按事件类型给出规则化的中性一句话（回购 / 问询 / 监管 / 宏观数据 / 盘中异动…共 20 类映射），卡片上标「一句话」而不是「AI 一句话」，不假装用了大模型。一句话与详细分析**各自过一遍合规红线**：哪一段越线就单独回退那一段，不连累另一段。
4. **AI 分析** —— 配置了 `DEEPSEEK_API_KEY` 时，把「本条 + 其它源头报道」一起交给 DeepSeek，按 ① 事件要点 ② 多源印证（各源是否一致、有无补充或口径差异；单一来源必须写明「待其它渠道确认」）③ 关注点 三段写 60–120 字中性分析。只能用给出的事实，禁止编造数据、禁止荐股；**单一来源却声称「已获多方证实」的输出会被拒收**。未配置或调用失败时降级为规则化分析（印证情况 + 按事件类型给出的核对要点），卡片上标「分析」而不是「AI 分析」，不假装用了大模型。

模型对每条情报的输出格式（`octopus/ai.py` 的 `NEWS_ANALYSIS_PROMPT`，`octopus/enrich.py` 按编号解析成两段）：

```
1. 一句话：公司自己掏 400 亿真金白银回购，等于管理层觉得现在这个价不贵。
   分析：事件要点：公司公告拟以集中竞价方式回购股份……
```

模型没按新格式写「一句话」时，整段仍按分析处理，开头那一句由规则化文本兜底——老格式的输出不会因为新增字段而失效。

```
标题  宁德时代(300750) 关于以集中竞价方式回购公司股份的公告
AI 一句话  公司自己掏 400 亿真金白银回购，等于管理层觉得现在这个价不贵。
现价  宁德时代 300750  251.30  +4.12%
多源 3  同一新闻的其它源头
      · 东方财富  宁德时代：拟以不超过400亿元回购股份  09-07 10:21
      · 证券时报（Google News）  宁德时代披露400亿元回购方案  09-07 10:12
      · 财联社（Google News）  宁德时代拟斥资不超400亿回购 创A股纪录  09-07 10:20
      · 证券之星  宁德时代（300750）10点02分触及涨停板  09-07 10:02（同标的，待核对）
AI 分析  事件要点：公司公告拟以集中竞价方式回购股份。多源印证：东方财富、证券时报、
        财联社三方口径一致，均为不超过400亿元，财联社补充「创A股纪录」；证券之星
        同日涨停为同标的盘中信号，是否与回购直接相关待核对。关注点：回购价格上限
        与实施期限以公告原文为准。
```

页面上的实际排版：`标题` → **`一句话`（深底高亮，全页最突出）** → `现价` → `源摘要` → `多源` → `AI 分析` → `时间与标签`。
...
多源印证的外部检索可配置（`config.yml` / 环境变量 / `--crossref`）：
...
tests/                # 453 个测试，全离线（含多源印证、一句人话、合并排版与单条上限回归测试）
└── fixtures/         # 真实接口响应样本
```
...
453 个测试全部离线运行（用真实抓取的响应样本做 fixture），不依赖网络。

**情报抓取**：时间解析的各种畸形格式（含 RSS 的 RFC 822 时间）、过滤逻辑、去重、降级路径、HTML 转义、
条数上限语义（0 = 全量）、每条新闻的一句人话与 AI 分析（代码提取 / 东财→Yahoo 降级 /
「一句话 + 分析」两段解析 / 规则化兜底 / 荐股措辞拒绝 / 单一来源却声称多方证实的拒收 /
高亮块排在现价与分析之前 / HTML 转义）、手动主题分析推送（一对一语义 / 独立网页 / 令牌保护 / DeepSeek 提炼）与端到端流程。

**多源印证**（`test_crossref.py`，39 个）：同标的同事件判为同一事件、同标的不同事件只判「同标的」且要求时间相近、
同一源头永不互相印证、快照类条目（板块热力/人气榜）不参与、标的名很长时剥掉再比重合度、
统计局「居民消费价格」与媒体「CPI」的别名归一、外部检索结果的时间校验（无时间/未来/相差过久一律丢弃）、
标的不同或事件不同的结果丢弃、同一家源头排除、`off` 模式零网络请求、`auto` 模式连续失败熔断、
`on` 模式不熔断、每轮检索条数上限、XSS 转义。
