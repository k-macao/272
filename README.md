...
推送样式：浅灰底色 + 深色正文。每条情报开头有一句**突出显示**的「AI 一句话」——证券投资专家口吻的大白话结论；
下面依次是 **现价**（能识别出标的时）、**同题报道与网上相似观点**，以及结构化 **AI 分析**（事件重塑 / 利弊挖掘 / 深度溯源 / 多维推演（含偏多偏空概率）/ 事实核查）。
...
## 每条情报：现价 + 多源观点 + AI 分析（一句人话合并其中，排在最后）

抓取、时间校验、去重完成之后、渲染推送之前，对**每一条**留下的情报做四件事。失败只让该条缺字段，不会丢掉已经校验过的内容。

1. **现价与数据上下文** —— 从 `extra.code` / 标题里的 `（600519）`、`300750.SZ`、公告证券代码、以及内置词典里的证券简称提取标的，批量取东财实时行情；东财失败或漏报再降级 Yahoo Finance。转债源内已有的 `price` 作为最后兜底。识别不出标的或行情缺失时**整行不展示**，绝不编造价格。上证指数 `000001` 与平安银行同码，只在「上证/沪指」语境下走指数。
2. **多源报道 + 网上相似观点** —— 让证券 AI 下判断前先有多个可核对视角（`octopus/crossref.py`）：
   - **本轮跨源匹配（离线，始终执行）**：十个源同一轮抓回来的条目互相比对。「共同标的 + 共同事件词」（回购/增持/问询函/涨停/CPI…共 25 类）判定为**同一事件**；只共享标的、事件不同且间隔 3 小时内的判定为**同标的**（卡片标「待核对」，不冒充同一事件）。比对标题重合度时先剥掉标的名与代码，避免不同事件被公司名称误抬相似度。
   - **外部新闻/观点检索（在线，可关）**：用 Google News / Bing News RSS 组合检索「标的 + 事件 + 观点/解读/影响」，同时收集同题事实报道与网上相似观点。带「观点、解读、影响、机构、券商、研报、风险、机会」等信号且确实命中同一标的/事件的结果才归为**相似观点**；保存搜索页公开摘要和规则相关度，不会假装读取链接后的全文。
   - 所有结果继续经过时间与相关性校验：**无发布时间、来自未来、与事件相差超过 36 小时、标的或事件不一致、同一家源头自我转载**均丢弃。最多展示 6 条，先给事实报道和相似观点各预留 3 个位置，空位再由高相关材料补齐，避免大量转载挤掉独立观点。每轮最多检索 12 条，单次 6 秒超时不重试，连续失败自动熔断。
   - 找不到就如实标 **「单一来源 / 观点样本不足」**，绝不凑数；少于两个不同观点来源时，AI 也不得写成「市场共识」。
3. **一句人话（与分析合并成一块，排在每条新闻最后）** —— 深底 + 荧光绿左边条，一句话讲清「这条消息意味着什么」，扫一眼就知道轻重。要求以**证券投资专家的口吻说大白话**（25–40 字），可以写短线偏多/偏空/中性，但不能写成操作指令；荐股、目标价、收益承诺同样禁止，**单一来源却写「已获多方证实」的句子会被拒收**。未配置 `DEEPSEEK_API_KEY` 或调用失败时，按事件类型给出规则化一句话，卡片上标「一句话」而不是「AI 一句话」；事件类型认不出来时**不再输出通用兜底句**（「先把这条消息说了什么看清楚」这类对任何新闻都成立的话），整行留空、卡片上不显示。一句话与详细分析**各自过一遍合规红线**，一段越线不会连累另一段。
4. **AI 分析（与一句人话合并成一块，排在每条新闻最后）** —— DeepSeek 收到的不是孤立标题，而是「新闻事实 + 源站发布时间 + 已核验现价/涨跌幅 + 本地词典匹配的行业板块/概念 + 同题报道 + 网上相似观点」。模型身份固定为**证券分析专家**，每条严格按五步输出：**【事件重塑】【利弊挖掘】【深度溯源】【多维推演】【事实核查】**。

   | 模块 | 要回答的问题 |
   | --- | --- |
   | **事件重塑** | 谁（公司/交易所/部委等行为主体，不是转载媒体）→ 对谁（标的/行业）→ 发生了什么（动作、金额、数量、时点）→ 进展到哪一步。剥离标题里的形容与情绪词，只留可核对的事实 |
   | **利弊挖掘** | 谁相对受益、谁相对承压（公司、上下游、同业、板块或全市场），利与弊各自的**量级与时效** |
   | **深度溯源** | 为什么会走到这一步：直接触发因素 → 再往上游追一层（政策/供需/资金/公司治理/行业周期）→ 同类事件通常怎样演进；**必须区分已披露事实与推测** |
   | **多维推演** | 至少覆盖短线情绪与资金、基本面与业绩、政策与监管三条线，再给未来 **1–5 个交易日**的偏多/偏空情景概率（格式 `偏多 62% / 偏空 38%`，两者必须合计 100%），并给出最关键验证点 |
   | **事实核查** | 关键要素（主体、金额、时间、代码）有无出处；几条同题报道、几条相似观点（少于 2 个来源必须写「观点样本不足」）；还有哪些缺口未核实 |

   概率是有限证据下的主观权重，不是统计预测、目标价或收益承诺；证据不足时应接近 50%/50%。

输出仍受事实与合规约束：只能引用输入里的数字、词典匹配、搜索标题与公开摘要；不能把「同标的消息」冒充同一事件，不能假装读过搜索链接全文。偏多/偏空概率不合计 100%（或概率没写在【多维推演】里）、五模块缺项、荐股措辞越线、或单一来源却声称多方证实时，模型分析会被拒收。未配置 API Key 或调用失败时，系统按事件类型给出**同样五模块**的规则化分析，并注明「规则情景权重，非统计预测」；卡片只标「分析」，不假装用了 AI。

**兜底条款只写有内容的部分，没内容 / 分析不出就不显示**：规则化分析逐模块判断手上到底有没有真材料 ——

- 认不出的字段直接省略，不再写「事件归类：未归类」「行业板块：未识别」「相关概念：未识别」「时间待核」这类占位文本；
- 没命中事件类型时，【利弊挖掘】【深度溯源】【多维推演】整块不写，也不再给出「偏多 50% / 偏空 50%」的默认档（认不出类型时这个概率不是中性判断，只是噪音）；只剩【事件重塑】+【事实核查】两块真内容时也照实显示；
- 事件类型、标的、行业板块与概念**一个都匹配不上**时（例如「关于召开 2025 年第一次临时股东大会的通知」这类通知），整条分析为空串，卡片上**不显示**「分析」这一块，绝不用兜底条款凑字数；
- 渲染层再兜一次：模块标签后面没有正文的行不渲染，正文整体为空的块 / 卡（含主题因子分析的解读卡）整块不显示。

**AI 没内容时改问「资深新闻总编」**：证券五模块只适合可研判的事件，遇到通知类、名单类、纯提示类情报往往写不出东西（或被合规红线拒收）。
这时对**剩下的条目**再问一次模型，换总编视角把同一条情报重塑成结构化极简简报（`octopus/ai.py` 的 `NEWS_BRIEF_PROMPT`）：

| 模块 | 要回答的问题 |
| --- | --- |
| **【核心快讯】** | 一句话（**50 字以内**）概括发生了什么。不照抄标题，先找重点、从几个不同角度提炼，再合并成一句能被扫读的话 |
| **【关键要素】** | 清单列出**时间 / 地点 / 核心涉事方 / 起因**，由模型判断归类；材料里没写明的项写「未提及」，不猜不编 |
| **【发展脉络】** | 按时间顺序梳理该事件目前已经历的 **3–5 个主要发展阶段**（①②③）：从材料里找关键词线索，一步一步推理串联；材料只支撑得起 1–2 个阶段就只写 1–2 个，靠推理补出的衔接标注「（据材料推断）」 |

总编简报**不做多空研判、不给概率、不给目标价**，只重整已有材料，因此几乎每条都说得出话；
它同样过一遍合规红线（荐股措辞、单一来源却称多方证实一律拒收），并额外要求【核心快讯】必须真的写出来，否则整条丢弃。
收稿优先级：**证券五模块 AI 分析 → 总编极简简报 → 规则化五模块（有真内容时）→ 整块不显示**。
简报里「核心快讯」这一句已经顶上一句人话的位置，所以此时规则化的通用一句话会被清掉，一块里不会两种口径打架；
块级标签如实区分体裁：模型五模块标 **「AI 分析」**、模型总编简报标 **「AI 简报」**、规则化标 **「分析」**，都不假装用了 AI。

每轮日志会给出各体裁条数与「分析不出不显示 N 条」的计数（`enrich_news()` 返回的
`ai` / `brief` / `rule` / `analysis_skipped` / `headline_skipped`），便于核对这一轮走了哪条路径。

模型对每条情报的输出格式（`octopus/ai.py` 的 `NEWS_ANALYSIS_PROMPT`，`octopus/enrich.py` 按编号解析成两段）：

```
1. 一句话：大额回购短线偏多，但真正分量取决于执行规模和节奏。
   分析：【事件重塑】宁德时代→自身A股→400亿元回购、上限280元/股、期限12个月，待股东大会通过；板块电力设备、概念锂电池；【利弊挖掘】受益：现有股东与流通筹码；承压：希望现金投向扩产的资金方；利的量级看实际执行比例，中短期可见；【深度溯源】直接触发：现金充裕且管理层认为股价低于内在价值；上游一层：动力电池价格战压估值、扩产放缓；同类事件通常先反映在情绪与成交，再回到进展公告（推测）；【多维推演】短线情绪偏暖、每股收益增厚有限、回购须走披露程序；偏多 62% / 偏空 38%：执行比例与价格上限待核验；验证点：每月回购进展公告；【事实核查】金额与期限见公司公告；2 个同题源头、1 个观点来源，观点样本不足；缺口：最终回购股数与注销比例未核实。
```

模型没按新格式写「一句话」时，整段仍按分析处理，开头那一句由规则化文本兜底（事件类型认不出来时这一行留空、不显示）——老格式输出不会因升级失效；五模块格式一旦出现概率，则会校验【多维推演】里的偏多与偏空是否合计 100%。

```
标题  宁德时代(300750) 关于以集中竞价方式回购公司股份的公告
现价  宁德时代 300750  251.30  +4.12%
多源 2 · 观点 2  同题报道与网上相似观点
      · 东方财富  宁德时代：拟以不超过400亿元回购股份  09-07 10:21
      · 证券时报（Google News）  宁德时代披露400亿元回购方案  09-07 10:12
      · 财联社（Google News）  如何看宁德时代回购影响  09-07 10:20（相似观点 · 相关度 82%）
      · 某券商媒体（Bing News）  回购对锂电龙头估值的三条影响  09-07 10:18（相似观点 · 相关度 68%）
09-07 10:12 · 回购
AI 分析                          ← 一句人话与分析合并成一块，排在新闻最后
      AI 一句话  大额回购短线偏多，但真正分量取决于执行规模和节奏。
      【事件重塑】宁德时代 → 自身 A 股 → 400亿元回购、上限280元/股、期限12个月，待股东大会通过
      【利弊挖掘】受益：现有股东与流通筹码；承压：希望现金投向扩产的资金方；量级看执行比例
      【深度溯源】现金充裕叠加股价低于内在价值；上游是电池价格战压估值、扩产资本开支持续放缓
      【多维推演】短线情绪偏暖、每股收益增厚有限、回购须走披露程序；偏多 62% / 偏空 38%；验证点为进展公告
      【事实核查】两家同题报道确认事实；两条观点都重视信心信号，但对执行效果仍有分歧
```

证券分析答不出来时，同一块换成总编极简简报（块级标签写作「AI 简报」）：

```
标题  关于召开 2025 年第一次临时股东大会的通知
09-19 21:04 · 公告
AI 简报
      【核心快讯】公司定于 9 月 28 日召开临时股东大会，审议董事会改选与章程修订。
      【关键要素】· 时间：2026-09-28 召开，通知于 09-19 发出
                · 地点：未提及
                · 涉事方：公司董事会、登记在册股东
                · 起因：董事会换届与章程修订需股东表决
      【发展脉络】① 董事会审议通过召开临时股东大会的议案
                ② 09-19 发出会议通知并披露议案清单
                ③ 股权登记日确定有权表决股东名册（据材料推断）
                ④ 09-28 现场表决并公告决议
```

页面上的实际排版：`标题` → `现价` → `源摘要` → `多源/观点` → `时间与标签` → **`AI 块`（一句人话 + 分析合并，深底高亮的一句人话打头）**。
模块分行展示，【多维推演】里的偏多用 A 股红、偏空用绿，简报的清单项与阶段也各占一行，便于手机扫读。
终端 `--dry-run` 摘要里同样把这一块打在每条最后（`AI分析` / `AI简报` / `分析`，后跟 `AI一句话` / `一句话`）。

正文里**不再渲染任何来源标题**（`▍东方财富 · 12 条` 这一级全部隐藏，十个源同此口径）：
每个源仍是一张独立卡片，卡片内条目直接以标题开头，相邻条目靠深浅交替底色区分。
来源的降级说明统一由页脚交代（`降级说明：东方财富…`），不在正文重复挂来源名。
手动主题分析、合并研报、主题因子分析那几种推送的章节标题不受影响。

同题报道与网上观点的外部检索可配置（`config.yml` / 环境变量 / `--crossref`）：

```yaml
crossref_mode: auto          # auto（默认，连不上自动熔断）/ on（每条候选都试）/ off（只做本轮跨源匹配）
crossref_max_items: 12       # 每轮最多对多少条做外部检索
crossref_timeout: 6          # 单次检索超时（秒），不重试
crossref_max_gap_hours: 36   # 外部报道与原条目发布时间相差超过此值即丢弃
```

```bash
python main.py --crossref off --dry-run   # 内网/离线机器：不联网检索，仍做本轮跨源匹配
```
...
## 夜间免打扰（凌晨 02:00 后暂停推送，07:00 起床）

北京时间 **02:00 至 07:00** 为免打扰时段（`quiet_start` – `quiet_end`）：

- **定时轮次整体跳过**：不抓取、不推送、不动去重账本，日志只留一行
  "本轮暂停抓取与推送"。GitHub Actions 的 cron 照常触发，进入后秒级退出（成功态）。
- **`--loop` 常驻模式直接睡到起床点**：02:00 后不再每 30 分钟空转，一觉到 07:00 再起轮。
...
```yaml
quiet_start: "02:00"   # 留空即关闭；两端相同也视为关闭
quiet_end: "07:00"     # 建议加引号，防止 YAML 把未加引号的 02:00 解析成六十进制数
```
...
quiet_start: "02:00"       # 夜间免打扰开始（北京时间），留空关闭
...
规则化解读同样遵循「没内容就不显示」：一个标的都没算出因子时【因子要点】整节不写（不留空标题），
因子来源未知时不留一对空括号；解读正文为空时整张解读卡不渲染。
...
├── state.py          # 跨轮次去重（原子写入）
├── http.py           # 重试、超时、UA 伪装
├── ai.py             # DeepSeek 大模型：内容提炼 + 主题因子报告解读 + 新闻逐条「一句话 + 分析」+ 总编极简简报
├── enrich.py         # 情报条目：现价、板块/概念匹配、五模块 AI 分析 → 总编简报 → 规则化兜底（说不出话就留空）
├── crossref.py       # 多源检索：同一事件匹配 + Google/Bing News 相似观点与公开摘要
├── render.py         # 浅灰底 + 深色字的 HTML 渲染（一句人话与分析合并成一块排在每条最后，相邻条目底色交替）
...
tests/                # 507 个测试，全离线（含多源观点、概率校验、总编简报兜底与排版回归测试）
└── fixtures/         # 真实接口响应样本
```
...
507 个测试全部离线运行（用真实抓取的响应样本做 fixture），不依赖网络。

**情报抓取与证券分析**：时间解析的各种畸形格式（含 RSS 的 RFC 822 时间）、过滤逻辑、去重、降级路径、HTML 转义、
条数上限语义（0 = 全量）、每条新闻的一句人话与五模块分析（代码提取 / 东财→Yahoo 降级 /
行业与概念词典匹配 /「一句话 + 分析」两段解析 / 规则化概率兜底 / 偏多偏空概率合计 100 校验 /
荐股措辞拒绝 / 单一来源却声称多方证实的拒收 / 五模块移动端排版 /
**兜底条款没内容或分析不出时整块不显示**（占位词不再出现、空模块行不渲染、空正文不出卡片）/
**总编极简简报兜底**（三模块断行排版、编号解析、缺【核心快讯】即丢弃、合规拒收后保留上一手文本、
顶掉规则化通用一句话、已有证券分析时不多问一次模型）、**一句人话与分析合并成一块排在每条最后**，
以及手动主题分析与端到端流程。

**多源报道与相似观点**（`test_crossref.py`）：同标的同事件判为同一事件、同标的不同事件只判「同标的」且要求时间相近、
带观点信号且命中同一标的/事件的结果归为「相似观点」、公开 RSS 摘要净化与规则相关度、
同一源头永不互相印证、快照类条目不参与、标的名很长时剥掉再比重合度、宏观指标别名归一、
外部结果时间校验（无时间/未来/相差过久一律丢弃）、标的或事件不同的观点丢弃、`off/auto/on` 模式、
熔断、每轮检索条数上限与 XSS 转义。
