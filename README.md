...
3. **一句人话（每条开头，页面突出显示）** —— 深底 + 荧光绿左边条，一句话讲清「这条消息意味着什么」，扫一眼就知道轻重。要求以**证券投资专家的口吻说大白话**（25–40 字），可以写短线偏多/偏空/中性，但不能写成操作指令；荐股、目标价、收益承诺同样禁止，**单一来源却写「已获多方证实」的句子会被拒收**。未配置 `DEEPSEEK_API_KEY` 或调用失败时，按事件类型给出规则化一句话，卡片上标「一句话」而不是「AI 一句话」；事件类型认不出来时**不再输出通用兜底句**（「先把这条消息说了什么看清楚」这类对任何新闻都成立的话），整行留空、卡片上不显示。一句话与详细分析**各自过一遍合规红线**，一段越线不会连累另一段。
4. **证券 AI 分析** —— DeepSeek 收到的不是孤立标题，而是「新闻事实 + 已核验现价/涨跌幅 + 本地词典匹配的行业板块/概念 + 同题报道 + 网上相似观点」。模型身份固定为**证券分析专家**，每条严格按五步输出：**【事件重塑】【利弊挖掘】【深度溯源】【多维推演】【事实核查】**。
...
每轮日志会给出「分析不出不显示 N 条」的计数（`enrich_news()` 返回的 `analysis_skipped` / `headline_skipped`），便于核对这一轮有多少条走了不显示的路径。
...
页面上的实际排版：`标题` → **`一句话`（深底高亮，全页最突出）** → `现价` → `源摘要` → `多源/观点` → `AI 分析五模块` → `时间与标签`。五个模块分行展示，【多维推演】里的偏多用 A 股红、偏空用绿，便于手机扫读。
终端 `--dry-run` 摘要里同样紧跟标题打印这一句（`AI一句话` / `一句话`）。
...
├── enrich.py         # 情报条目：现价、板块/概念匹配、五模块 AI/规则化证券分析
...
tests/                # 487 个测试，全离线（含多源观点、概率校验、一句人话与排版回归测试）
└── fixtures/         # 真实接口响应样本
```
...
487 个测试全部离线运行（用真实抓取的响应样本做 fixture），不依赖网络。
