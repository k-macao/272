...
| `DEEPSEEK_API_KEY` | DeepSeek 大模型 API Key（定时情报逐条一句人话 + AI 分析 + 手动主题分析提炼） | ❌ |
...
### 一、把 workflow 放进仓库（需要你手工复制一次）

推送本次改动的 GitHub App 没有 `workflows` 权限，
`.github/workflows/theme_analysis.yml` 推不上去，所以准备了**粘贴用过渡文件**：

```
deploy/github-workflows/theme_analysis.yml.txt   ← 打开它，复制分隔线之后的全部内容
```

**GitHub 网页操作**（最省事，不用拉代码）：

1. 仓库 → **Add file → Create new file**；
2. 文件名填 `.github/workflows/theme_analysis.yml`；
3. 打开 `deploy/github-workflows/theme_analysis.yml.txt`，
   把 `# ===== 分隔线：以下为正文 =====` **之后**的全部内容粘进去
   （分隔线之前是操作说明，不要粘）；
4. **Commit changes**。

**或者本地命令行**（`.yml` 已在仓库里，直接拷）：

```bash
git checkout arena/019ff535-272
mkdir -p .github/workflows
cp deploy/github-workflows/theme_analysis.yml .github/workflows/
git add .github/workflows/theme_analysis.yml
git commit -m "启用主题因子分析 workflow"
git push
```

> `.yml.txt` 分隔线之后的内容与 `.yml` **逐字节一致**，有测试
> （`tests/test_factor_pipeline.py::TestWorkflowTemplate`）盯着，
> 两者一旦脱节测试就会失败——不会出现「复制到的是旧版本」。
...
首次建议 `dry_run` 选 `true` 跑一个你熟悉的板块，核对个股清单和因子读数是否合理，
再改回 `false` 正式推送。
